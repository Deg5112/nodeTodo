...
//middleware that we pass into, this is the local strategy
...
//Routes start ..TODO split the above up into a require file
...
router.post('/register', function(req, res){
	...
	//   express validation, so express allows us to use the methods below
	...
	if(errors){
      ...
      if (userArray.length > 0) {
        ...
        res.redirect('/users/register');
          ...
          //if no errors, make a new user object with the schema of user created in model
...
//Facebook login
// Redirect the user to Facebook for authentication.  When complete, Facebook will redirect the user back to the app
      ...
      User.updateResetHash(email, hashString, function(err,mongoResponse){
        if(mongoResponse){
          ...
          if(mongoResponse.nModified == 1 && mongoResponse.n == 1){
            ...
            mailgun.messages().send(data, function (error, body) {
                console.log(body); //TODO flashbag message if false
            });
          ...
          if(mongoResponse.nModified == 0 && mongoResponse.n == 0){
            //it expired.. have a resend activation link page.. they enter their email, and send to a new route, that encrypts password and sends them a link, back to this route
